    ...
    if request.method == 'POST':
        ...
        if (search_curent_admin):
            ...
            if data['ok']:
                ...
                find_same_email_admin = mongo.db.admins.find_one(
                    {'email': data['email']})
    ...
    if request.method == 'POST':
        ...
        if data['ok']:
            ...
            if (search_admin and check_password_hash(search_admin['password'], data['password'])):
                ...
                search_admin['token'] = create_access_token(
                    identity=search_admin)
                search_admin['refresh'] = create_refresh_token(
                    identity=search_admin)
                output['status'] = True
    ...
    if request.method == 'GET':
        ...
        if (search_curent_admin):
            for single_user in mongo.db.admins.find():
                ...
                single_user['updatedAt'] = convertTimestampToDateTime(
                    single_user['updatedAt'])
                output['status'] = True
